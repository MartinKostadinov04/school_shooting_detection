/**
 * Shared incident-detail panel used by both the police dispatch console
 * and the school-side dashboard.
 *
 * Role-conditional bits:
 *   - VideoPathInput (the "submit a clip for visual confirmation" form) is
 *     police-only.  Schools never originate a video-inference job.
 *   - The CommunicationWindow's inline/floating layout is forced inline here
 *     so both roles see the per-incident chat embedded inside the panel.
 */
...
export function IncidentDetail({
  ...
  return (
    <div className="flex h-full flex-col gap-3 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
        ...
        <div className="rounded-md border border-border bg-surface p-3">
          <div className="mb-2 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {typeof incident.gunCount === "number" && incident.gunCount > 0 && (
                <span
                  className="inline-flex items-center rounded-sm border border-tactical-red/50 bg-tactical-red/10 px-1.5 py-0.5 font-mono text-[10px] font-bold uppercase tracking-widest text-tactical-red"
                  title="Peak number of simultaneously visible guns"
                >
                  ×&nbsp;{incident.gunCount}&nbsp;{incident.gunCount === 1 ? "GUN" : "GUNS"}
                </span>
              )}
          ...
          {incident.videoUrl ? (
            ...
            <div className="flex aspect-video items-center justify-center rounded-sm border border-dashed border-border bg-background/40 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
              {incident.source === "AUDIO-AI" &&
              !incident.timeline.some((t) => t.label === "Video scan negative")
                ? "Awaiting video segment from police"
                : "No video segment"}
            </div>
          )}
        </div>
      ...
      {/* Comms — forceInline so both roles see the per-incident chat
          embedded in the panel rather than as a floating bubble. */}
      <div className="h-80">
        <CommunicationWindow
          viewerRole={viewerRole}
          filterIncidentId={incident.id}
          forceInline
        />
      </div>
    </div>
